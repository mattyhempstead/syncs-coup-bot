...
class GameInfo:
    ...
    def deck_has_changed_since_turn(self, turn: int) -> bool:
        for turn_details in self.history[turn + 1:]:
            primary_action = turn_details[ActionType.PrimaryAction]
            ...
            primary_challenge_failed = (
                ActionType.ChallengePrimaryAction in turn_details and
                not turn_details[ActionType.ChallengePrimaryAction].successful
            )

            counter_challenge_failed = (
                ActionType.ChallengeCounterAction in turn_details and
                not turn_details[ActionType.ChallengeCounterAction].successful
            )

            if (
                exchanged
                or primary_challenge_failed
                or counter_challenge_failed
            ):
                return True
    ...
    def known_cards(
        self,
        exchanged_cards: Optional[tuple[list[Character], int]] = None,
    ) -> dict[Character, int]:
        """
        ...
        cards_to_add = [*self.own_cards]
        if exchanged_cards is not None:
            exchanged_cards_to_add, turn = exchanged_cards
            if not self.deck_has_changed_since_turn(turn):
                cards_to_add = [*cards_to_add, *exchanged_cards_to_add]
    ...
    def is_lying(
        ...
        known_cards = self.known_cards(exchanged_cards=exchanged_cards)
